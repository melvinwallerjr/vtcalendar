...
if (isset($_POST['event'])) {
	...
	if (isset($_POST['event']['defaultallsponsor'])) { $event['defaultallsponsor'] = true; }
	else { unset($event['defaultallsponsor']); }
	if (isset($_POST['event']['defaultdisplayedsponsor'])) { $event['defaultdisplayedsponsor'] = true; }
	else { unset($event['defaultdisplayedsponsor']); }
	if (isset($_POST['event']['defaultdisplayedsponsorurl'])) { $event['defaultdisplayedsponsorurl'] = true; }
	else { unset($event['defaultdisplayedsponsorurl']); }
}
else { unset($event); }
...
";
		if (is_string($result =& DBQuery($query))) {
			DBErrorBox('Error retrieving record from "' . SCHEMANAME . 'vtcal_event_public' . $result);
			exit;
		}
	...
	if (empty($_SESSION['AUTH_ISCALENDARADMIN']) && (!isset($_SESSION['AUTH_SPONSORID']) ||
	 $_SESSION['AUTH_SPONSORID'] != $e['sponsorid'])) {
		 redirect2URL($httpreferer);
		 exit;
	}
}
...
if (empty($event['sponsorid'])) { $event['sponsorid'] = $_SESSION['AUTH_SPONSORID']; }

// If the event is not repeating, make sure the end date is the same as the begin date.
if (!isset($repeat['mode']) || $repeat['mode'] == '0') {
	$event['timeend_year'] = isset($event['timebegin_year'])? $event['timebegin_year'] : date('Y');
	$event['timeend_month'] = isset($event['timebegin_month'])? $event['timebegin_month'] : date('n');
	$event['timeend_day'] = isset($event['timebegin_day'])? $event['timebegin_day'] : date('j');
}

// Otherwise, set up the repeating data.
if (isset($repeat['mode']) && $repeat['mode'] > 0 && !empty($repeat['interval1'])) {
...
if ($eventvalid && isset($savethis)) {
	// Save event into DB (if it is valid).
	require('changeeinfo-save.inc.php');
}
elseif ($eventvalid && isset($preview) && isset($check)) {
	// Display preview
...
{
	$eventFields = array('timebegin_year', 'timebegin_month', 'timebegin_day', 'timebegin_hour',
	 'timebegin_min', 'timebegin_ampm', 'timeend_year', 'timeend_month', 'timeend_day', 'timeend_hour',
	 'timeend_min', 'timeend_ampm', 'repeatid');
	foreach ($eventFields as $field) {
		echo '<input type="hidden" name="event[' . $field . ']" value="';
		if (isset($event[$field])) {
			if (strip_tags($event[$field]) == $event[$field]) { // contains no HTML
				$out = nl2br(HTMLSpecialChars(trim($event[$field])));
			}
			else { $out = trim($event[$field]); } // has HTML
			echo preg_replace("/\s\s+/", ' ', $out);
		}
		echo '" />' . "\n";
	}
	$repeatFields = array('mode', 'interval1', 'frequency1', 'interval2', 'frequency2modifier1',
	 'frequency2modifier2');
	foreach ($repeatFields as $field) {
		echo '<input type="hidden" name="repeat[' . $field . ']" value="';
		if (isset($repeat[$field])) {
			if (strip_tags($repeat[$field]) == $repeat[$field]) { // contains no HTML
				$out = nl2br(HTMLSpecialChars(trim($repeat[$field])));
			}
			else { $out = trim($repeat[$field]); } // has HTML
			echo preg_replace("/\s\s+/", ' ', $out);
		}
		echo '" />' . "\n";
	}
...
function passeventvalues($event, $sponsorid, $repeat)
{
	...
	$fields = array('sponsorid', 'title', 'wholedayevent', 'categoryid', 'description', 'location',
	 'webmap', 'price', 'contact_name', 'contact_phone', 'contact_email', 'displayedsponsor',
	 'displayedsponsorurl', 'showondefaultcal', 'showincategory');
	foreach ($fields as $field) {
		...
		if (isset($event[$field])) {
			...
			echo ($field == 'description')? htmlspecialchars($out, ENT_COMPAT, 'UTF-8') : $out;
		}
		echo '" />' . "\n";
	}
}
