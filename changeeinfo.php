...
require_once('session_start.inc.php');
  ...
  if ((isset($savethis) || isset($saveall)) && $eventvalid) {
    ...
    if ($repeat['mode']>=1 && $repeat['mode']<=2) { $repeatlist = producerepeatlist($event,$repeat); }
		...
		// update an existing event ("copy" is like adding a new event)
    if (isset($eventid) && !isset($copy)) {

			// before it was a single event
      if (empty($event['repeatid'])) {
        ...
        // the event is still a single event
        else {
      ...
      // it had recurrences before
        ...
        // the event had recurrences before but now is a single event
        else {
    ...
    // insert as new event(s) or copy an event
    else {
