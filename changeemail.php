...
require_once('application.inc.php');
	...
	if (isset($save)) {
		...
		if (checkemail($sponsor["email"])) { // email is valid
			// save email to DB
			$result = DBQuery("UPDATE ".TABLEPREFIX."vtcal_sponsor SET email='".sqlescape($sponsor_email)."' WHERE calendarid='".sqlescape($_SESSION['CALENDAR_ID'])."' AND id='".sqlescape($_SESSION["AUTH_SPONSORID"])."'" ); 
	...
	{ // read the sponsor's email from the DB
		$result = DBQuery("SELECT * FROM ".TABLEPREFIX."vtcal_sponsor WHERE calendarid='".sqlescape($_SESSION['CALENDAR_ID'])."' AND id='".sqlescape($_SESSION["AUTH_SPONSORID"])."'" ); 
		$sponsor = $result->fetchRow(DB_FETCHMODE_ASSOC,0);
