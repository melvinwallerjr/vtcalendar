...
require_once('session_start.inc.php');
	...
	if (isset($save)) {
		...
		if (checkemail($sponsor["email"])) { // email is valid
			// save email to DB
			$result = DBQuery("UPDATE vtcal_sponsor SET email='".sqlescape($sponsor_email)."' WHERE calendarid='".sqlescape($_SESSION['CALENDAR_ID'])."' AND id='".sqlescape($_SESSION["AUTH_SPONSORID"])."'" ); 
