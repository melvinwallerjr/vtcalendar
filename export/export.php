...
if (isset($_GET['adminexport']) || isset($_POST['adminexport'])) {
	...
	$FUNCINCLUDE = array_flip(explode(' ',
	 'dates dates-generic inputvalidation db-generic db-gets export misc'));
...
elseif (is_int($calendardata)) {
	if ($calendardata == 0) {
		...
		outputErrorMessage('Too many records returned when attempting to retrieve' .
		 ' the calendar information (' . $calendardata . ').');
	}
}
// If the calendar requires authentication to view, then exit.
elseif ($calendardata['viewauthrequired'] > 0) {
	outputErrorMessage('The \'' . $CalendarID . '\' calendar requires authentication' .
	 ' to be viewed, so it cannot be used by the export script.');
}
...
switch($FormData['format']) {
	case 'rss':
		...
		echo GenerateRSS($result, $CalendarID, $calendardata['name'] .
		 ': ' . $calendardata['title'], BASEURL);
		break;
	case 'rss1_0':
		if (!isset($_GET['raw'])) { header('Content-Type: text/xml'); }
		echo GenerateRSS1_0($result, $CalendarID, $calendardata['name'] .
		 ': ' . $calendardata['title'], BASEURL);
		break;
	case 'rss2_0':
		if (!isset($_GET['raw'])) { header('Content-Type: text/xml'); }
		echo GenerateRSS2_0($result, $CalendarID, $calendardata['name'] .
		 ': ' . $calendardata['title'], BASEURL, BASEURL.EXPORT_PATH . '?' . http_build_query($_GET));
		break;
	case 'xml':
		if (!isset($_GET['raw'])) { header('Content-Type: text/xml'); }
		echo GenerateXML($result, $CalendarID, $calendardata['name'] .
		 ': ' . $calendardata['title'], BASEURL);
		break;
	case 'ical':
		if (!isset($_GET['raw'])) {
	...
	case 'vxml':
		...
		break;
	case 'html':
		if ($FormData['jshtml']) { header('Content-type: application/x-javascript; charset=utf-8'); }
