...
WHERE
	id='" . sqlescape($calendarid) . "'
");
	if (is_string($result)) { return $result; }
	elseif ($result->numRows() != 1) { return $result->numRows(); }
	else { return $result->fetchRow(DB_FETCHMODE_ASSOC, 0); }
}

function getCalendarList($type=null)
{ // returns a list of available calendars in various data formats
	$result =& DBQuery("
SELECT
	id,
	name
...
");
	...
	for ($i=0; $i < $result->numRows(); $i++) {
		...
		$id = htmlspecialchars(html_entity_decode($calendar['id']), ENT_COMPAT, 'UTF-8');
		$name = htmlspecialchars(html_entity_decode($calendar['name']), ENT_COMPAT, 'UTF-8');
		if ($type == 'select') {
			$calendarsList .= '<option value="' . $calendar['id'] . '"' .
			 ((isset($_SESSION['CALENDAR_ID']) && $_SESSION['CALENDAR_ID'] == $calendar['id'])?
			 ' selected="selected"' : '') . '>' . $calendar['name'] . '</option>' . "\n";
		}
		elseif ($type == 'links') {
			$calendarsList .= '<li><a href="main.php?calendarid=' . $calendar['id'] . '">' .
			 ((isset($_SESSION['CALENDAR_ID']) && $_SESSION['CALENDAR_ID'] == $calendar['id'])?
			 '<b>' . $calendar['name'] . '</b>' : $calendar['name']) . '</a></li>' . "\n";
		}
		else {
			$calendarsArray[] = array(htmlspecialchars($calendar['id'], ENT_COMPAT, 'UTF-8'),
			 htmlspecialchars($calendar['name'], ENT_COMPAT, 'UTF-8'));
		}
	}
	return ($type == 'select' || $type == 'links')? $calendarsList : $calendarsArray;
}
...
");
	...
	if ($result->numRows() == 1) {
		...
		$_SESSION['CALENDAR_LANGUAGE'] = $calendar['language'];
...
");
	if (is_string($result)) { return $result; }
	if ($result->numRows() == 1) { $record =& $result->fetchRow(DB_FETCHMODE_ASSOC, 0); }
...
SELECT
	c.name
FROM
	" . SCHEMANAME . "vtcal_sponsor AS s,
	" . SCHEMANAME . "vtcal_calendar AS c
WHERE
	s.id='" . sqlescape($sponsorid) . "'
	AND
	c.id=s.calendarid
");
	if ($result->numRows() > 0) {
...
WHERE
	e.calendarid='" . sqlescape($CalendarID) . "'
	AND
	e.categoryid=c.id
	AND
	e.sponsorid=s.id
		...
		if (isset($FormData['timebegin'])) {
			...
			if (isset($FormData['timeend'])) {
				if (isValidInput($FormData['timeend'], 'int_gte1')) {
					$query .= " AND e.timeend <= '" . sqlescape(date('Y-m-d',
					 strtotime($FormData['timeend'] . ' day', $BeginTicks))) . " 23:59:59' ";
				}
				else {
