...
if (!isset($sponsorid)) { echo 'INVALID_SPONSOR_ID:', $_GET['sponsorid']; }
else {
	$result =& DBQuery("
SELECT
	*
...
");
	if (is_string($result)) { echo 'ERROR:' . $result; }
		...
		if ($result->numRows() == 0) { echo 'SPONSOR_ID_NOTFOUND:', $_GET['sponsorid']; }
		else {
			$sponsor =& $result->fetchRow(DB_FETCHMODE_ASSOC, 0);
			if ($type == 'name') { echo $sponsor['name']; }
			elseif ($type == 'url') { echo $sponsor['url']; }
		}
	}
}
?>