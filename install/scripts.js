function ToggleDependant(variableid)
{
	if (document.getElementById) {
		...
		if (objCheckbox && objRow) {
			if (objCheckbox.checked) { objRow.style.display = ""; }
			else { objRow.style.display = "none"; }
		}
	}
}
...
function AddCheckDSNLink()
{
	if (document.getElementById) {
		...
		if (objDBStringExtra) {
			objDBStringExtra.innerHTML = "<a href=\"#\" onclick=\"CheckDSN();" +
			 "return false;\">[Check Connection]<\/a>";
		}
	}
}
...
function SetSponsorDefault(typeNum)
{
	...
	if (document.getElementById) {
		...
		if (objSelectedSponsorID) {
			...
			var returnValue = httpGet("getsponsorinfo.php?sponsorid=" + sponsorID +
			 "&type=" + type, typeNum, SetSponsorDefaultProcessor);
			return !returnValue;
		}
	}
	return false;
