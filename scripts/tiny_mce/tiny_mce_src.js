...
/*				ed.onPreProcess.add(function(ed, o) {
			...
			ed.onKeyUp.add(function() {
				// extends status bar to provide word/character/code count
				var v = tinyMCE.activeEditor.getContent(),
				 n = tinymce.DOM.get(tinyMCE.activeEditor.id + "_path_row");
				var s = v.replace(/(<([^>]+)>)/ig, "");
				 tinymce.DOM.setHTML(n, n.innerHTML.replace(/(<var>.*)/, "") +
				 "<var> (" + s.split(" ").length + " Words, " + s.length +
				 " Chars, " + v.length + " Code)<\/var>");
			});
		},
