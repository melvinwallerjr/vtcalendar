...
if (isset($_POST['event'])) {
	...
	if (!isset($_POST['event']['webmap']) || !setVar($event['webmap'], $_POST['event']['webmap'], 'webmap')) { unset($event['webmap']); }
...
pageheader(lang('edit_template', false), 'Update');
contentsection_begin(lang('edit_template'));

echo '
<form action="updatetinfo.php" method="post">
<input type="hidden" name="templateid" value="' . htmlspecialchars($templateid, ENT_COMPAT, 'UTF-8') . '" />';

if (!isset($check)) { $check = 0; }
inputtemplatedata($event, $_SESSION['AUTH_SPONSORID'], $check, $template_name);

echo '
<p><input type="submit" name="savetemplate" value="' . htmlspecialchars(lang('ok_button_text', false), ENT_COMPAT, 'UTF-8') . '" />
&nbsp;
